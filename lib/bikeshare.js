...
const REMOVE_STRS = [', Portland, Oregon', ', Portland'];
...
function loadGbfsStations(filePath, agencyId, agencyName, layerId) {
  return fs.createReadStream(filePath, 'utf8')
      ...
      .pipe(through.obj(function(rec, _, callback) {
        ...
        var address = utils.cleanAddressString(rec.address, REMOVE_STRS);
        var altName = agencyName + ' ' + rec.station_id;

        var doc = utils.makePeliasRecord(
        LAYER_ID, id, rec.name,
        rec.lat, rec.lon,
        null, address
        );
...
module.exports.loadGbfsStations = loadGbfsStations;
