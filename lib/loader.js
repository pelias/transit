 ...
 */
function load(fullPath, dataType, agencyId, agencyName) {
    var stream = null;
    try {
        ...
        if(dataType == 'stops') {
            stream = stops.loadGtfsStops(fullPath, agencyId, agencyName, dataType);
        } else if(dataType == 'pr') {
            stream = landmarks.loadTransitLandmarks(fullPath, agencyId, agencyName, dataType);
        } else if(dataType == 'bikeshare') {
            //stream = bikeshare.loadGbfsRacks(fullPath, agencyId, agencyName, dataType);
            stream = null;
        } else {
